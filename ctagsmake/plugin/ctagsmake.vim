...
fun! s:QuickFixOut(id,data,event)
	echo 'tags make finished'
endfun
fun! s:GoToOldwin()
   exec "close"
...
fun! s:RgGrepOnline()
   ...
   exec "close"
   call win_gotoid(s:oldwinid)
   let grepcmd="cd ".grepfrom
   exec grepcmd
   if IsVim()
       call job_start('ctags -R --exclude=*.html --exclude=*.css')
   else
       call jobstart('ctags -R ',{'on_exit':function('s:QuickFixOut')})
    endif
endfun
fun! s:Map_Keys()
