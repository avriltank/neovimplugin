...
" Core functions {{{
	...
	function Mycompare(one,two)
		let one=a:one
		let two=a:two
	   return one[0] == two[0] ? 0 : one[0] > two[0] ? 1 : -1
	endfunction
	function! s:Paixu(yuan,line,col)
		let juli=[]
		for every in a:yuan
			let yy=abs(every[0]-a:line)
			"let xx=abs(every[1]-a:col)
			let temp=[]
			call add(temp,yy)
			"call add(temp,xx)
			call add(temp,every)
		...
		let juli=sort(juli,'Mycompare')
		let result=[]
		for everytemp in juli
			call add(result,everytemp[1])
		endfor
		return result
	endfunction
	function! s:EasyMotion(regexp, direction, visualmode, mode) " {{{
		...
		try
			...
			" Find motion targets {{{
				...
				while 1
					let pos = searchpos(a:regexp, 'b', line('w0'))
				...
				let targets=s:Paixu(targets,line,col)		
